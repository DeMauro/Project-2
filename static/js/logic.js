...
d3.select("#exampleFormControlSelect1")
  .selectAll("option")
...
function updateFilters() {
  ...
  d3.select("#exampleFormControlSelect2")
    .selectAll("option")
    .exit()
  ...
  d3.select("#exampleFormControlSelect3")
    .selectAll("option")
    .exit()
...
function ModelGraph(data) {
  ...
  var layout = {
    autosize: true,
    title: `Average Cost by ${makeValue}`,
    xaxis: { title: "$ Avg. Cost" },
    // yaxis: { title: "Models" },
  };
  Plotly.newPlot("viz-two", data, layout);
...
function BuildMap(data) {
  ...
  myMap.setView([37.0902, -95.7129], 4); // resets the view // Create a new marker cluster group
  var markers = L.markerClusterGroup(); // Loop through data
  for (var i = 0; i < data.length; i++) {
    console.log(data[i]); // Add a new marker to the cluster group and bind a pop-up
    markers.addLayer(
      L.marker([data[i]["lat"], data[i]["lng"]]).bindPopup(
        "<center><h6>" +
          data[i]["Make"] +
          " " +
          data[i]["Model"] +
          ...
          d3.format(",")(data[i]["craigslist_price"]) +
          " MPG" +
          data[i]["mpg"] +
          "</center></h6>"
      )
